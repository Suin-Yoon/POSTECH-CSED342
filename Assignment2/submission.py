...
def extractWordFeatures(x):
    ...
    for word in words:
        if word in dict_feature_vector:
            dict_feature_vector[word] += 1
        else:
            dict_feature_vector[word] = 1

    return dict_feature_vector
...
def learnPredictor(trainExamples, testExamples, featureExtractor, numIters, eta):
    ...
    for i in range(numIters):
        for x, y in trainExamples:
            ...
            grad = {f: features[f] * error for f in features}
...
class MLPBinaryClassifier:
    ...
    def forward(self, x):
        ...
        # Hidden layer
        z1 = x @ self.W1 + self.b1  # Affine transformation
        a1 = sigmoid(z1)

        # Output layer
        z2 = a1 @ self.W2 + self.b2  # Affine transformation
        pred = sigmoid(z2)
    ...
    def loss(pred, target):
        ...
        """
        # BEGIN_YOUR_ANSWER
        return (-1) * (target * np.log(pred) + (1 - target) * np.log(1 - pred))
        # END_YOUR_ANSWER
    ...
    def backward(self, pred, target):
        ...
        # dW1와 db1은 중간층의 가중치와 편향에 대한 기울기.
        dW1 = self.x.T @ dZ1
        db1 = np.sum(dZ1, axis=0, keepdims=True)

        # 기울기를 담은 dictionary 반환
    ...
    def train(self, X, Y, epochs=100, learning_rate=0.1):
        ...
        for epoch in range(epochs):
            for i in range(X.shape[0]):
                ...
                # 역전파
                grad = self.backward(pred, self.Y)
                # 가중치 및 편향 업데이트
