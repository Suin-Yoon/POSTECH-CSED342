...
class WordSegmentationProblem(util.SearchProblem):
    ...
    def succAndCost(self, state):
        ...
        result = []
        for endState in range(state + 1, len(self.query) + 1):
            action = self.query[state:endState]
            cost = self.unigramCost(action)
            result.append((action, endState, cost))
        return result
        # END_YOUR_CODE
...
class VowelInsertionProblem(util.SearchProblem):
    ...
    def succAndCost(self, state):
        ...
        if index < len(self.queryWords):
            possibleWords = self.possibleFills(self.queryWords[index])
            if not possibleWords: # if no words
                possibleWords.add(self.queryWords[index])
                
            for word in possibleWords:
                nextState = (index + 1, word)
                cost = self.bigramCost(prevWord, word)
                result.append((word, nextState, cost))
    ...
    def succAndCost(self, state):
        ...
        for i in range(index + 1, len(self.query) + 1):
            for word in self.possibleFills(self.query[index : i]):
                newstate = (i, word)
                cost = self.bigramCost(prevWord, word)
                result.append((word, newstate, cost))
        return result
        # END_YOUR_CODE
...
def AStarMazeSearch(start: tuple, goal: tuple, moveCost: Callable[[tuple, str], float],
    ...
    totalCost = 0
    moveCostDict = moveCost.moveCost
    for action in ucs.actions:
        totalCost += moveCostDict[action]
    return totalCost
    # END_YOUR_CODE
