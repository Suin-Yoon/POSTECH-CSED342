...
def create_nqueens_csp(n = 8):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if(i == j): # 서로 다른 두 퀸에 대해서만 하도록..
                continue
            # 같은 행에 있을 수 없음
            csp.add_binary_factor(variables[i], variables[j], lambda x, y: x != y)
            # 대각선 상에 있을 수 없음
            csp.add_binary_factor(variables[i], variables[j], lambda x, y: abs(x-y) != abs(i-j))
    # END_YOUR_ANSWER
    return csp
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            # BEGIN_YOUR_ANSWER
            #raise NotImplementedError            
            consistent_values_cnt = {}
            for var in self.csp.variables:
                if var not in assignment:
                    consistent_values_cnt[var] = len([val for val in self.domains[var] if self.get_delta_weight(assignment, var, val) > 0])
                
            min_length = min(consistent_values_cnt.values())
            for var in self.csp.variables:
                if var in consistent_values_cnt and consistent_values_cnt[var] == min_length:
                    return var
            # END_YOUR_ANSWER
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    # BEGIN_YOUR_ANSWER
    #raise NotImplementedError
    sum_var = ('Sum', name, 'aggregated')
    
    if len(variables) == 0:
        csp.add_variable(sum_var, [0])
        return sum_var
    else:
...
def create_lightbulb_csp(buttonSets, numButtons):
    ...
    for buttonIndex in range(numButtons):
        csp.add_variable(('button', buttonIndex), [True, False])

    for bulbIndex, buttonSet in enumerate(buttonSets):
        controlling_buttons = [('button', buttonIndex) for buttonIndex in buttonSet]
        # 합이 홀수인 경우만 전구가 켜지도록 설정
        sum_var = get_sum_variable(csp, ('sum_bulb', bulbIndex), controlling_buttons, len(buttonSet))
        # 합이 홀수이어야 한다는 constraint 추가
        csp.add_unary_factor(sum_var, lambda x: x % 2 == 1)
    # END_YOUR_ANSWER
    return csp
